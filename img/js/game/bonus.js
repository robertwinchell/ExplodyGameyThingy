...
Game.winBomb = function() {
	...
	bomb.style.left = ((Game.GEM_SIZE * x) + (5 * (x + 1))) + 'px';
	bomb.style.top = ((Game.GEM_SIZE * y) + (5 * (y + 1))) + 'px';
	bomb.id = 'tile' + y + '_' + x;
...
Game.winDoubleBomb = function() {
	...
	dbomb.style.left = ((Game.GEM_SIZE * x) + (5 * (x + 1))) + 'px';
	dbomb.style.top = ((Game.GEM_SIZE * y) + (5 * (y + 1))) + 'px';
	dbomb.id = 'tile' + y + '_' + x;
	itemToReplace = get('#tile' + y + '_' + x);
	if (itemToReplace != null && itemToReplace.parentNode) {
		get('#grid').removeChild(itemToReplace);
