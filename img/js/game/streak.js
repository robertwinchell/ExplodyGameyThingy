...
Game.checkStreak = function(gem) {
	...
	var isMatchedLT = false;
	var isFourGems = false;
	var isFiveGems = false;
	/*
	*Check if gems are matched L or T
	*/
	console.log(streak);
	if(Object.getLength(streak) >2)
	{
		var nRowCount = 0;
		...
		for(var i = 0 ; i < Game.GRID_SIZE ; i ++)
		{
			...
			{
				if(Object.getLength(streak[i]) == 3)
				{
					isMatchedLT = true;
					console.log("LT");
				}
	...
	/*
	*Check if 5 gems are matched in a row
	*/
	if(Object.getLength(streak) == 4)
	{
		isFourGems = true;
		console.log("four");
	}

	else if(Object.getLength(streak) == 5)
	{
		isFiveGems = true;
		console.log("five");
	}
		...
		for(var i = 0 ; i < Game.GRID_SIZE ; i ++)
		{
			if(streak[i] != undefined)
			{
				if(Object.getLength(streak[i]) == 4)
				{
					isFourGems = true;
					console.log("four");
				}
				if(Object.getLength(streak[i]) == 5)
				{
					isFiveGems = true;
					console.log("five");
				}
	...
	*/
	...
	if (Object.getLength(streak) > 0) {
		...
		//Create an Bomb
		if(isMatchedLT == true || isFourGems == true)
			Game.winBomb();

		//Create an DoubleBomb
		if(isFiveGems == true)
		{
			// Game.winDoubleBomb();
			Game.winBomb();
...
Game.generateGems = function(x) {
	...
	for (var i = Game.GRID_SIZE - 1; i >= 0; i--) {
		...
		if (currentGem == null) {
			...
			if (check) value = 0;
			y = -1 * quantity;
			gem = new Game.Gem(parseInt(x), y, value, Game.GEM_SIZE);
