...
Game.mainLoop = setInterval(function() {
	...
	if(Game.GAME_MODE == 1 || Game.GAME_MODE == 2) Game.updateTimer();
...
Game.onMoveComplete = function(gems) {
	...
	//When the Game mode is level and the goal has been reached, we go to the next level
	if(Game.GAME_MODE == 2 || Game.GAME_MODE == 4)
	{
...
document.getElementById('t_casual').onclick = function()
{
...
document.getElementById('t_level').onclick = function()
{
	document.getElementById('content').style.display = 'block';
	document.getElementById('startGame').style.display = 'none';
	Game.GAME_MODE = 2;
...
document.getElementById('u_casual').onclick = function()
{
	//hide the minute and second timer
	document.getElementById('minuteBox').classList.add('unlimit');
	document.getElementById('secondBox').classList.add('unlimit');
	//show the unlimit text
	document.getElementById('unlimit').classList.remove('unlimit');
	document.getElementById('content').style.display = 'block';
	document.getElementById('startGame').style.display = 'none';
	Game.GAME_MODE = 3;
	Game.STARTED = true;
...
document.getElementById('u_level').onclick = function()
{
    //hide the minute and second timer
    document.getElementById('minuteBox').classList.add('unlimit');
    document.getElementById('secondBox').classList.add('unlimit');
...
document.getElementById('shuffle_bt').onclick = function()
{
	Game.emptyGrid();
	Game.createGrid();
}
